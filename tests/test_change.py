...
class TestChangeset(test.CreatesSchema):
    ...
    def test_parents_with_fget(self):
        # Delete all accounts for this test, so we can focus on
        # parents with fget fields.
        for account in db.Account.find():
            db.execute(account.t.delete())
        ...
        # Changing Fred's gender from UNASSIGNED to Male results in
        # the new related entity, Male, being updated.
        tx = fred.t.update(gender=male)
        db.execute(tx)
        changes = tx.sys.changes
        assert list(changes) == [
            (UPDATE, 'Person', fred.sys.oid),
            (UPDATE, 'Gender', male.sys.oid),
            ]
        summary = tx.sys.summarize()
        assert summary.creates == dict()
        assert summary.deletes == dict()
        assert summary.updates == dict(
            Person = set([fred.sys.oid]),
            Gender = set([male.sys.oid]),
            )
        # Changing Fred's gender from Male to Female results in the
        # old related entity, Male, being updated, and the new related
        # entity, Female, being updated.
        tx = fred.t.update(gender=female)
        db.execute(tx)
        changes = tx.sys.changes
        assert list(changes) == [
            (UPDATE, 'Person', fred.sys.oid),
            (UPDATE, 'Gender', male.sys.oid),
            (UPDATE, 'Gender', female.sys.oid),
            ]
        summary = tx.sys.summarize()
        assert summary.creates == dict()
        assert summary.deletes == dict()
        assert summary.updates == dict(
            Person = set([fred.sys.oid]),
            Gender = set([male.sys.oid, female.sys.oid]),
            )
        # Unassigning Fred's gender results in the old related entity,
        # Female, being updated.
        tx = fred.t.update(gender=UNASSIGNED)
        db.execute(tx)
        changes = tx.sys.changes
        assert list(changes) == [
            (UPDATE, 'Person', fred.sys.oid),
        ...
        assert summary.updates == dict(
            Person = set([fred.sys.oid]),
            Gender = set([female.sys.oid]),
            )
        # Creating Barney results in the related entity, Male, being
        # updated.
        tx = db.Person.t.create(name='Barney Rubble', gender=male)
        barney = db.execute(tx)
        changes = tx.sys.changes
        assert list(changes) == [
            (CREATE, 'Person', barney.sys.oid),
            (UPDATE, 'Gender', male.sys.oid),
            ]
        summary = tx.sys.summarize()
        assert summary.creates == dict(Person=set([barney.sys.oid]))
        assert summary.deletes == dict()
        assert summary.updates == dict(Gender=set([male.sys.oid]))
        # Deleting Barney results in the previously related entity,
        # Male, being updated.
        tx = barney.t.delete()
        db.execute(tx)
        changes = tx.sys.changes
        assert list(changes) == [
            (DELETE, 'Person', barney.sys.oid),
            (UPDATE, 'Gender', male.sys.oid),
            ]
        summary = tx.sys.summarize()
        assert summary.creates == dict()
        assert summary.deletes == dict(Person=set([barney.sys.oid]))
        assert summary.updates == dict(Gender=set([male.sys.oid]))
    ...
    def test_children(self):
        ...
        female = db.execute(db.Gender.t.create(code='F', name='Female'))
        fred = db.Person.findone(name='Fred Flintstone')
        db.execute(fred.t.update(gender=male))
        barney = db.execute(db.Person.t.create(
            name='Barney Rubble', gender=male))
        # Changing Male's name to 'MALE' results in both Fred and
        # Barney being updated as well since the label for Male
        # probably changed.
        tx = male.t.update(name='MALE')
        db.execute(tx)
        changes = tx.sys.changes
        # Sort updates for comparison purposes.
        assert sorted(changes) == [
            (UPDATE, 'Gender', male.sys.oid),
            (UPDATE, 'Person', fred.sys.oid),
            (UPDATE, 'Person', barney.sys.oid),
            ]
        summary = tx.sys.summarize()
        assert summary.creates == dict()
        assert summary.deletes == dict()
        assert summary.updates == dict(
            Gender = set([male.sys.oid]),
            Person = set([fred.sys.oid, barney.sys.oid]),
            )
