...
class TestDatabaseMongo(BaseDatabase):
    ...
    def test_schevo_key_in_root(self):
        """A database has a `SCHEVO` identifier in its root."""
        collection = self.db.backend._collection
        assert collection.find_one({'SCHEVO': True}) is not None
